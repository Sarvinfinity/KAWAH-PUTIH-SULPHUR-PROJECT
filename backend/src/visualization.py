...
def create_comparison_plot(df_original, df_expanded, feature, feature_name):
    ...
    filename = os.path.join(WORKSPACE_ROOT, "outputs", "plots", f"{feature.lower()}_comparison.png")
...
def main():
    ...
    BACKEND_DIR = os.path.dirname(SRC_DIR)
    WORKSPACE_ROOT = os.path.dirname(BACKEND_DIR)
    df_original = pd.read_csv(os.path.join(WORKSPACE_ROOT, 'data', 'raw', 'all_data_ts.csv'))
    df_expanded = pd.read_csv(os.path.join(WORKSPACE_ROOT, 'data', 'processed', 'expanded_24H_all_data.csv'))
