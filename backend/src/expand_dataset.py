...
def process_node(df_node, node_id, time_step=30, future_seconds=17*3600):
    ...
    from atmospheric_correction import apply_atmospheric_physics_correction
...
def main():
    ...
    in_file = os.path.join(WORKSPACE_ROOT, 'data', 'raw', 'all_data_ts.csv')
    print(f"Loading original dataset '{in_file}'...")
    df_original = pd.read_csv(in_file)
    ...
    # Save to CSV
    out_file = os.path.join(WORKSPACE_ROOT, 'data', 'processed', 'expanded_24H_all_data.csv')
    ...
    plt.legend()
    plt.grid(True)
    plot_out = os.path.join(WORKSPACE_ROOT, 'outputs', 'plots', 'node_expansion_plot.png')
